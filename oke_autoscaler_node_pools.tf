...
resource "oci_containerengine_node_pool" "fk_oke_autoscaler_node_pool_2" {
  depends_on         = [oci_containerengine_node_pool.fk_oke_node_pool]
  count              = var.virtual_node_pool ? 0 : var.autoscaler_enabled ? var.autoscaler_node_pool_count : 0
  cluster_id         = oci_containerengine_cluster.fk_oke_cluster.id
  compartment_id     = var.compartment_ocid
  kubernetes_version = var.k8s_version
  name               = "Autoscaler${var.pool_name}${count.index+2}"
  node_shape         = var.node_shape
...
resource "oci_containerengine_node_pool" "fk_oke_autoscaler_node_pool_3" {
  depends_on         = [oci_containerengine_node_pool.fk_oke_node_pool]
  count              = var.virtual_node_pool ? 0 : var.autoscaler_enabled ? var.autoscaler_node_pool_count : 0
  cluster_id         = oci_containerengine_cluster.fk_oke_cluster.id
  compartment_id     = var.compartment_ocid
  kubernetes_version = var.k8s_version
  name               = "Autoscaler${var.pool_name}${count.index+3}"
  node_shape         = var.node_shape
